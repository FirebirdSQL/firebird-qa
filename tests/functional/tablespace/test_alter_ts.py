...
"""
ID:          functional.tablespace.alter_ts
TITLE:       
DESCRIPTION:
FBTEST:      functional.tablespace.alter_ts
"""
...
def test_1(act: Action, tmpdir: Path):
    ...
    act.isql(switches=['-q'], input=f"ALTER TABLESPACE TS1 SET FILE TO '{ts2_file}'; commit;")
    ...
    select * from test_table;    
    """
    act.isql(switches=['-q'], input=after_rename_script)
    stdout += act.clean_stdout
