...
"""
ID:          transactions.read-consist-sttm-restart-on-delete-02
TITLE:       READ CONSISTENCY. Check creation of new statement-level snapshot and restarting changed caused by DELETE. Test-02.
DESCRIPTION:
  Initial article for reading:
      ...
      * session 'worker' ("LONG" in TK article) has mission:
              delete from test where id < 0 or id >= 3 order by id; // using TIL = read committed read consistency
...
FBTEST:      functional.transactions.read_consist_sttm_restart_on_delete_02
NOTES:
[28.07.2022] pzotov
...
def test_1(act: Action, fn_worker_sql: Path, fn_worker_log: Path, fn_worker_err: Path, capsys):
    ...
    for checked_mode in('table', 'view'):
        ...
        with act.db.connect() as con_lock_1, act.db.connect() as con_lock_2:
            ...
            con_lock_1.execute_immediate( f'update {target_obj} set id=id where id = 5' % locals() )
            ...
            with fn_worker_log.open(mode='w') as hang_out, fn_worker_err.open(mode='w') as hang_err:
                ...
                con_lock_1.commit()
                con_lock_1.execute_immediate( f'update {target_obj} set id=id where id = -2' % locals() )
                ...
                # and it should see this new value and restart its statement:
                con_lock_2.commit()
                con_lock_2.execute_immediate( f'insert into {target_obj}(id) values(-3)' % locals() )
                con_lock_2.commit()
