#coding:utf-8
#
# id:           functional.transactions.read_consist_sttm_restart_on_merge_04
...
#          set count on;
#  
#          merge into %(target_obj)s t -- THIS MUST HANG BECAUSE OF LOCKERs
#          using (select * from %(target_obj)s where id <=2 order by id DESC rows 4) s on s.id=t.id
#          when matched then
#              update set t.id = -t.id
#          when not matched then
#              insert(id,x) values(1000 + s.id, 1000 + s.x);
...
@pytest.mark.xfail
def test_read_consist_sttm_restart_on_merge_04_1(db_1):
