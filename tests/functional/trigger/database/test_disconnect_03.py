...
"""
ID:          trigger.database.trigger_on_disconnect_03
TITLE:       ON DISCONNECT trigger must fire if attachment is deleted from MON$ATTACHMENTS by another attachment.
DESCRIPTION: 
    ...
    During that, we  create 'admin' connection which issues 'DELETE FROM MON$ATTACHMENTS WHERE ...'
    This *must* cause firing trigger with adding record to 'detach_audit' table (and, according to doc, engine
    always starts autonomous transaction for that).
...
def test_1(act: Action, tmp_worker: User):
    ...
    worker_remains_alive = False
    with act.db.connect() as con_admin, act.db.connect(user = tmp_worker.name, password = tmp_worker.password) as con_worker:
        con_admin.execute_immediate(f'delete from mon$attachments where mon$attachment_id = {con_worker.info.id}')
        con_admin.commit()
        cur_admin = con_admin.cursor()
        cur_admin.execute(f'select mon$attachment_id from mon$attachments where mon$attachment_id = {con_worker.info.id}')
        for r in cur_admin:
            worker_remains_alive = True
    assert not worker_remains_alive, '### FAIL ### Could not delete attachment of non-privileged user'
