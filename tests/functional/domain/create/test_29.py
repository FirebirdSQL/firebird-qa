...
DESCRIPTION: 
    Domain creation based on BLOB datatype with SEGMENT SIZE specification
    NB: segment size must be smallint.
NOTES:
    [06.10.2023] pzotov
	...
	4. Check TWO domain sub-types: text and binary. For text we try to add diactritic characters from WIN1250 charset.
"""
...
test_script = f"""
    ...
    create domain dm_test blob SEGMENT SIZE 32761 character set win1250 collate WIN_CZ;
    ...
    create domain dm_test blob sub_type binary SEGMENT SIZE 32763;
    recreate table test(s dm_test);
    commit;
    select v.* from v_domain_info v;
