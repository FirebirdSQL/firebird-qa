...
test_script = f"""
    ...
    -- create domain dm_test varchar({DM_SIZE}) character set win1250 CHECK(VALUE SIMILAR TO '%range%') collate WIN_CZ_CI_AI;
    create domain dm_test varchar({DM_SIZE}) character set win1250 CHECK(VALUE SIMILAR TO '%EEIIDD%') collate WIN_CZ_CI_AI;
    ...
    insert into test (s) values('{CHK_TXT}') returning cast(s as varchar(2048)) as inserted_data;
    commit;
"""
