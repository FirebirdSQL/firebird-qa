...
TITLE:       CREATE DOMAIN. Check ability to declare using syntax BLOB(seglen, num_subtype)
DESCRIPTION: Domain creation based on BLOB with "(segment_len, num_datatype)" specification.
NOTES:
    [06.10.2023] pzotov
    ...
    2. Currently one may NOT to specify character set after "create domain dm_test blob(<segment_len>,1)": error "token unknown" is issued.
...
test_script = f"""
    ...
    create domain dm_test blob(12347,1) -- character set win1250 collate WIN_CZ
    ;
    ...
    create domain dm_test blob(32101,0);
    recreate table test(s dm_test);
    commit;
    select v.* from v_domain_info v;
