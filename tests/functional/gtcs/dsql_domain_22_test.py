#coding:utf-8
#
# id:           functional.gtcs.dsql_domain_22
# title:        GTCS/tests/DSQL_DOMAIN_22. Verify result of ALTER DOMAIN with changing NOT NULL flag and CHECK constraints when a table exists with field based on this domain.
# decription:   
...
#               
#                   After this we try to change domain definition by adding NOT NULL clause - and it must fail because of existing record with null.
#                   Finally, we replace CHECK constraint so that its new expression will opposite to previous one, and try again to insert record with DEFAULT value.
#                   It must fail because of new domain CHECK violation.
#               
#                   This is performed separately for each datatype (smallint, int, ...).
#               
#                   ::: NB-1 :::
#                   Test uses datatypes that did appear only in FB 4.0: INT128, DECFLOAT and TIME[STAMP] WITH TIME ZONE. For this reason only FB 4.0+ can be tested.
#               
#                   ::: NB-2 :::
...
#               
#                   Checked on 4.0.0.2425 (Windows), 4.0.0.2422 (Linux)
#                   
#                
# tracker_id:   
# min_versions: ['4.0']
...
substitutions_1 = [('After line.*', ''), ('X_BLOB_20.*', ''), ('X_BLOB_21.*', ''), ('X_BLOB_22.*', ''), ('DM_FVALID.*', ''), ('DM_FDEFAULT.*', ''), ('0.0000000000000000', '0.000000000000000'), ('X_DATE                          20.*', 'X_DATE 20'), ('validation error for column "TEST"."X_DATE", value .*', 'validation error for column "TEST"."X_DATE"')]
